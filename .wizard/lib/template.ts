...
async function renderAndCopyFile(
  ...
  if (file.split('.').pop() == 't') {
    ...
    const destPath = fullDestinationPath.split('.').pop() == 't' ? fullDestinationPath.substring(0, fullDestinationPath.length - 2) : fullDestinationPath
...
const template = {
  ...
  getTemplateStructure: async (
    ...
    const basePath = path.join('.', 'templates')
    ...
    const isTsAndPreact = config.language == CustomScriptLanguage.TYPESCRIPT && config.framework === CustomScriptFramework.PREACT
    const isJsAndPreact = config.language == CustomScriptLanguage.JAVASCRIPT && config.framework === CustomScriptFramework.PREACT
    const isJsAndVanilla = config.language == CustomScriptLanguage.JAVASCRIPT && config.framework === CustomScriptFramework.VANILLA
    const isTsAndVanilla = config.language == CustomScriptLanguage.TYPESCRIPT && config.framework === CustomScriptFramework.VANILLA
    ...
    if (config.collectsUserData) {
      ...
      const deleteUserDataCallbackFileName = `delete-user-data-callback.${isTypeScript ? 't' : 'j'
      }s`
      templatePaths.push({
    ...
    templatePaths.push({
      ...
      destinationPath: path.join('.', 'custom-script', 'components',
          `Card.${isSvelte ? 'svelte' : ''}${isVue ? 'vue' : ''}${isJsAndVanilla ? 'js' : ''}${isTsAndVanilla ? 'ts' : ''}${isTsAndPreact ? 'tsx' : ''}${isJsAndPreact ? 'jsx' : ''}`),
      isFile: true,
    })
    ...
    if (isTypeScript) {
      ...
      const frontendTypeDefPath = path.join(languagePath, 'frameworks', 'types.d.ts')
      templatePaths.push({
